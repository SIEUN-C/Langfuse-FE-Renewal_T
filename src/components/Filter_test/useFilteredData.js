            ...
            tempData = tempData.filter((item) => {
                return activeFilters.every((filter) => {
                    ...
                    if (itemValue === null || itemValue === undefined) {
                        if (filter.operator === "is null") return true;
                        if (filter.operator === "is not null") return false;
                        return false;
                    }
                    ...
                    switch (filter.type) {
                        case "string":
                                    case "stringObject": {
                                      ...
                                      const traceString = String(itemValue).toLowerCase();
                                      const filterString = String(filterValue).toLowerCase();
                                      if (filter.operator === "=") return traceString === filterString;
                                      if (filter.operator === "contains") return traceString.includes(filterString);
                                      if (filter.operator === "does not contain") return !traceString.includes(filterString);
                                      if (filter.operator === "starts with") return traceString.startsWith(filterString);
                                      if (filter.operator === "ends with") return traceString.endsWith(filterString);
                                      return true;
                                    }
                                    case "number":
                                    case "numberObject": {
                                    ...
                                    case "stringOptions": { // 보통 단일 값 필터링에 사용 (예: environment)
                                      ...
                                      if (filter.operator === "any of") return filterValues.includes(traceString);
                                      if (filter.operator === "none of") return !filterValues.includes(traceString);
                                      return true;
                                    }
                                    case "arrayOptions": { // 보통 배열 값 필터링에 사용 (예: tags)
                                      ...
                                      if (filter.operator === "none of") return !filterValues.every(v => itemValues.includes(v));
                                      if (filter.operator === "all of") return filterValues.every(v => itemValues.includes(v));
                                      return true;
                                    }
                                    case "null": {
                                      if (filter.operator === "is null") return itemValue === null || itemValue === undefined;
                                      if (filter.operator === "is not null") return itemValue !== null && itemValue !== undefined;
                                      return true;
                                    }
                                    default:
                                      return true;
